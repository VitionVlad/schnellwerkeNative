...
impl Scene{
    ...
    pub fn load_from_obj(eng: &mut Engine, path: &str, material: Material, voxelize: bool, voxel_size: f32) -> Scene{
        ...
        for i in 0..mdst.len(){
            for j in 0..mdtx.len(){
                if obj.mtl.matnam[j] == obj.matnam[i]{
                    fobj.push(Object::new(eng, mdst[i], material, vec![mdtx[j]], super::render::render::MeshUsage::ShadowAndDefferedPass, true, obj.obn[i].clone(), obj.rwvertices[i].clone()));
                    break;
                }
            }
        }
    ...
    pub fn load_from_gltf(eng: &mut Engine, path: &str, material: Material, voxelize: bool, voxel_size: f32) -> Scene{
        ...
        for i in 0..gltfsc.objs.len(){
            ...
            scn.objects.push(Object::new(eng, tobj, material, vec![ldmt[gltfsc.objs[i].material]], super::render::render::MeshUsage::ShadowAndDefferedPass, true, gltfsc.objs[i].name.clone(), gltfsc.objs[i].rwvertices.clone()));
